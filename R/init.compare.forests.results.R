...
# use.matrix: num.obs x num.trees matrix of 0s and 1s (indicating whether an observation was used for a particular tree)
...
init.compare.forests.results <- function(num.obs, num.trees) {
  ...
  use.matrix <- matrix(0, num.obs, num.trees)
  variance.honest <- rep(0, num.obs)
  variance.standard <- rep(0, num.obs)
  list(pred.honest = pred.honest, pred.standard = pred.standard, pred.honest.matrix = pred.honest.matrix, pred.standard.matrix = pred.standard.matrix, use.matrix = use.matrix, variance.honest = variance.honest, variance.standard = variance.standard)
}