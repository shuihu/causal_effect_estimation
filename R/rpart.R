...
# we need to add propensity score p in rpart.R
# we need to add cv.option in rpart.R
rpart <-
 #   function(formula, data, weights, subset, na.action = na.rpart, method,
  function(formula, data, weights, subset, na.action = na.rpart, method, cv.option,
#             model = FALSE, x = FALSE, y = TRUE, parms, control, cost, ...)
              model = FALSE, x = FALSE, y = TRUE, parms, p, control, cost, ...) # p := propensity score
# ... is only for those arguments in rpart.control
# so if I want to add args, one idea is in rpart.control, or just change the whole code by adding and reducing some args.             
{
      # 2015.8.7
      Call <- match.call()
