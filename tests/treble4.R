...
library(causalTree)
control <- causalTree.control(maxsurrogate=0, cp=1e-15, xval=0)
...
fit1b <- causalTree(Kyphosis ~ Age + Number + Start, data=kyphosis, 
               control=control,
	       weights=wts,
               parms=list(prior=c(.7,.3), 
                 loss=matrix(c(0,1,2,0),nrow=2,ncol=2)))
...
xgct <- rep(1:10, length=nn)[order(pseudo)]
xgct2<- rep(xgct, wts)
...
tempc <- causalTree.control(minsplit=2, xval=xgct2, maxsurrogate=0, cp=.039)
#  Direct: replicate rows in the data set, and use unweighted
fit2 <- causalTree(Kyphosis ~ Age + Number + Start, data=kyphosis[temp,], 
               control=tempc, 
               parms=list(prior=c(.7,.3), 
                          loss=matrix(c(0,1,2,0),nrow=2,ncol=2)))
#  Weighted
tempc <- causalTree.control(minsplit=2, xval=xgct, maxsurrogate=0, cp=.039)
fit2b <- causalTree(Kyphosis ~ Age + Number + Start, data=kyphosis, 
