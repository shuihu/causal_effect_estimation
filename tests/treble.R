...
tempc <- causalTree.control(maxsurrogate=0, cp=0, xval=0)
...
fit1b <- causalTree(Surv(pgtime, pgstat) ~ age + eet + g2+grade+gleason +ploidy,
...
xgct <- rep(1:10, length=146)[order(pseudo)]
xgct2<- rep(xgct, wts)
...
fit2 <- causalTree(Surv(pgtime, pgstat) ~ age + eet + g2+grade+gleason +ploidy,
	      control=causalTree.control(minsplit=2, xval=xgct2, cp=.025),
	      data=stagec[temp,], method='poisson')

#  Weighted
fit2b<- causalTree(Surv(pgtime, pgstat) ~ age + eet + g2+grade+gleason +ploidy,
	      control=causalTree.control(minsplit=2, xval=xgct, cp=.025),
