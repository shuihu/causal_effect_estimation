...
fit4 <- causalTree(income ~ population + region + illiteracy +life + murder +
                   ...
                   control=causalTree.control(minsplit=10, xval=xgct))
wts <- rep(3, nrow(mystate))
fit4b <-  causalTree(income ~ population + region + illiteracy +life + murder +
                        hs.grad + frost , mystate,
                   control=causalTree.control(minsplit=10, xval=xgct), weights=wts)
...
xx1 <- causalTree(y ~ x1 + x2, dummy, minsplit=4, xval=0)
xx2 <- causalTree(y ~ x1 + x2, dummy, weights=rep(2,10), minsplit=4, xval=0)
...
xgct <- rep(1:10, length=nn)
xgct2<- rep(xgct, wts)
tempc <- causalTree.control(minsplit=2, xval=xgct2, maxsurrogate=0,
		       maxcompete=3, maxdepth=3)
#  Direct: replicate rows in the data set, and use unweighted
fit5 <-  causalTree(income ~ population + region + illiteracy +life + murder +
                        hs.grad + frost , data=mystate[temp,], control=tempc)
#  Weighted
tempc <- causalTree.control(minsplit=2, xval=xgct, maxsurrogate=0,
		       maxcompete=3, maxdepth=3)
fit5b <-  causalTree(income ~ population + region + illiteracy +life + murder +
