causalForest_Thai_2 <- function(X, Y, W, num.trees, subsample.fraction, Jsample.fraction = 0.5, mtry = ceiling(ncol(X)/3), nodesize = 1) {
  ...
  num.obs <-nrow(X)
  causalForest.honest <- causalForest:::init.causalForest(X, Y, W, num.trees)
  ...
  for (tree.index in 1:num.trees) {
    ...
    tree.standard <- causalTree(Y ~ ., data = data.frame(X = X[train.idx,], Y = Y[train.idx]), 
                          treatment = W[train.idx], method = "anova2", alpha=.75, 
                          cp = 0, minbucket = nodesize, cv.option = "TOT", 
                          split.option = "CT", xval = 0)
    
    tree.honest <- causalForest:::reestimate.tau(tree.standard, Y[reestimation.idx], data.frame(X = X[reestimation.idx,]), W[reestimation.idx])
