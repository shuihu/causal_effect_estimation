...
simu.fun = function(seed.idx, d) {
  ...
  forest1 = randomForest(X[W==1, ], Y[W==1], sampsize = floor(nrow(X[W==1, ])/2), ntree = ntree, keep.inbag = TRUE)
  predictions1 = predict(forest1, X.test)
  forest1.ci = randomForestInfJack(forest1, X.test, calibrate = TRUE)

  forest0 <- randomForest(X[W==0, ], Y[W==0], sampsize = floor(nrow(X[W==0, ])/2), ntree = ntree, keep.inbag = TRUE)
...
raw_rets = lapply(start.time + 1:NREP, simu.fun, d);

proc.time();

save.image(paste0("Test_Two_Forests/output_paper1113_3_two_forests_0.5_", d, ".RData"))
