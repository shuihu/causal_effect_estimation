...
rpart(SEXP ncat2, SEXP method2, SEXP opt2,
      SEXP parms2, SEXP p2, SEXP xvals2, SEXP xgrp2,
      SEXP ymat2, SEXP xmat2, SEXP wt2, SEXP ny2, SEXP cost2, SEXP xvar2)
{
    ...
    // pass the variance of predictors into rp:
    rp.xvar = REAL(xvar2);
    ...
    for (i = 0; i < n; i++) {
      ...
      for (j = 0; j <  rp.num_y; j++){
        if (fabs(rp.ydata[i][j]) > temp2) temp2 = fabs(rp.ydata[i][j]);        
      }
      	dptr += rp.num_y;
    }
    rp.max_y = temp2;
    ...
    // add rp.maxx_y inside the evaluation funciton
