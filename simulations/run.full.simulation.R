...
run.full.simulation <- function(num.replications = 1000, designs = 9, model.names = c('ST', 'TT', 'TOT_split_xval_rpart', 'TOT_xval', 'CT'), os.names = c('os.to', 'os.m', 'os.infeasible'), num.obs.per.set = 500, num.vars.per.obs = 10, propensity = 0.5, xvals = 10, is.honest = TRUE, is.honest0.5 = FALSE, is.dishonest2 = FALSE, data.path, full.tree.path.prefix, seed) {
  ...
  all.tree.stats <- init.all.tree.stats(num.replications, designs, model.names, c('dishonest.in.dishonest.conf.intv.95', 'dishonest.in.dishonest.conf.intv.90', 'honest.in.honest.conf.intv.95', 'honest.in.honest.conf.intv.90', 'honest.in.dishonest.conf.intv.95', 'honest.in.dishonest.conf.intv.90', 'dishonest.in.honest.conf.intv.95', 'dishonest.in.honest.conf.intv.90', 'test.in.dishonest.conf.intv.95', 'test.in.dishonest.conf.intv.90', 'test.in.honest.conf.intv.95', 'test.in.honest.conf.intv.90', 'weighted.test.in.dishonest.conf.intv.95', 'weighted.test.in.dishonest.conf.intv.90', 'weighted.test.in.honest.conf.intv.95', 'weighted.test.in.honest.conf.intv.90', 'is.full.tree', 'num.leaves', os.names))
  all.winning.models <- init.all.winning.models(designs, model.names, os.names)
  ...
  if (missing(data.path)) {
    ...
    test.XW <- read.input.for.all.designs(paste(data.path, "/test_design_", sep = ""), designs)
    test.Y <- read.output.for.all.designs(paste(data.path, "/test_design_", sep = ""), designs)
    match.indices <- read.match.observations.for.all.designs(paste(data.path, "/test_design_", sep = ""), designs)
    counterfactual.test.XW <- read.counterfactual.input.for.all.designs(paste(data.path, "/test_design_", sep = ""), designs)
    counterfactual.test.Y <- read.counterfactual.output.for.all.designs(paste(data.path, "/test_design_", sep = ""), designs)
  }
  for (repl in 1:num.replications) {
    ...
    if (missing(data.path)) {
      ...
      train.split.XW <- read.input.for.all.designs(paste(data.path, "/train_split_repl_", as.character(repl), "_design_", sep = ""), designs)
      train.estimation.XW <- read.input.for.all.designs(paste(data.path, "/train_estimation_repl_", as.character(repl), "_design_", sep = ""), designs)
      train.split.Y <- read.output.for.all.designs(paste(data.path, "/train_split_repl_", as.character(repl), "_design_", sep = ""), designs)
      train.estimation.Y <- read.output.for.all.designs(paste(data.path, "/train_estimation_repl_", as.character(repl), "_design_", sep = ""), designs)
    }
